...
person.message = () => {
  let { name, gender, age, height, weight, walkedMeters } = person;
  const correctGender = gender === "Feminino" ? "a" : "o";
  const ageSingularOrPlural = age === 1 ? "ano" : "anos";
  const meterSingularOrPlural = walkedMeters === 1 ? "metro" : "metros";
  return `Oi. Eu sou ${correctGender} ${name}, tenho ${age} ${ageSingularOrPlural}, ${height} metros de altura, peso ${weight} quilos e, só hoje, eu já caminhei ${walkedMeters} ${meterSingularOrPlural}.`;
};
