...
const numberGreaterThan50 = randomNumbers.find((elem) => elem > 50);
...
console.log(peopleCopy.sort().reverse());
...
const message = ingredients
  .map((ingredient) => `${ingredient} ${ingredient.endsWith("a") ? "cozida" : "cozido"}`)
  .join(", ");

console.log(message); //vinho cozido, tomate cozido, cebola cozida, cogumelo cozido
...
// const total = topBrazilmovies.reduce((acc, {peopleAmount, distributedBy}) => {
...
const moviesNames = topBrazilmovies.reduce((acc, {title}) => acc + `<li>${title}</li>`,'');

ulContainer.innerHTML = moviesNames;
